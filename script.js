...
/* unlock audio once (browser policy) */
// ✅ unlock BOTH sounds on first real user gesture
window.addEventListener(
  "pointerdown",
  () => {
    [sound, noSound].forEach((a) => {
      a.load();
      a.volume = a === noSound ? 0.35 : 0.6;
      a.play()
        .then(() => {
          a.pause();
          a.currentTime = 0;
...
// overlay that forces one real click
const overlay = document.createElement("div");
overlay.style.cssText = `
  position:fixed;inset:0;z-index:99999;
  display:flex;align-items:center;justify-content:center;
  background:rgba(0,0,0,0.25);backdrop-filter:blur(2px);
  color:#fff;font:700 28px Arial; text-shadow:0 6px 18px rgba(0,0,0,.35);
  cursor:pointer;
`;
